...
class CheckDuplicateFilenames(Filter):
    ...
    def _do_process(self, data):
        ...
        for item in make_list(data):
            if item.audio_name in self._names:
                msg = "File name already encountered: %s" % item.audio_name
                if item.source is not None:
                    msg += "\n- full path: %s" % item.source
                if item.audio_name in self._paths:
                    msg += "\n- previous file: %s" % self._paths[item.audio_name]
                ...
                if self.action == DUPLICATE_ACTION_IGNORE:
                    ...
                    self.logger().warning(msg)
                    self.logger().warning("Dropping file: %s" % item.audio_name)
                    item = None
