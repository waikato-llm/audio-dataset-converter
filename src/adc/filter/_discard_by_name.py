    ...
    def _do_process(self, data):
        ...
        for item in make_list(data):
            ...
            if len(self._names) > 0:
                if audio_name in self._names:
                    if not self.invert:
                        self.logger().info("Skipping based on name match: %s" % item.audio_name)
                        add = False
                else:
                    if self.invert:
                        self.logger().info("Skipping based on no name match (invert): %s" % item.audio_name)
                        add = False

            # check against regexps
            if add:
                for regexp in self._regexps:
                    if regexp.fullmatch(audio_name) is not None:
                        if not self.invert:
                            self.logger().info("Skipping based on regexp match: %s" % item.audio_name)
                            add = False
                            break
                    else:
                        if self.invert:
                            self.logger().info("Skipping based on no regexp match (invert): %s" % item.audio_name)
                            add = False
                            break
