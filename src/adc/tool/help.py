...
from adc.registry import register_plugins, available_pipeline_plugins, available_data_formatters, available_phonemizers, \
    available_watermarks, available_watermark_detectors, available_watermarkers

HELP = "adc-help"

_logger = logging.getLogger(HELP)

INDEX_TITLE_DEFAULT = "audio-dataset-converter plugins"
...
def output_help(custom_class_listers: List[str] = None, excluded_class_listers: Optional[List[str]] = None,
    ...
    elif plugin_type == PLUGIN_TYPE_WATERMARKS:
        available = available_watermarks()
    else:
    ...
    if index_file is not None:
        ...
        if plugin_type == PLUGIN_TYPE_PIPELINE:
            ...
            _add_plugins_to_index("Watermarkers", available_watermarkers(), help_format, plugin_lines)
            _add_plugins_to_index("Watermark Detectors", available_watermark_detectors(), help_format, plugin_lines)
        else:
            raise Exception("Unhandled plugin type: %s" % plugin_type)
